...
use rustix::fs::{renameat, openat, unlinkat, fchmod, AtFlags, OFlags, Mode};
        ...
        for e in rd.flatten() {
            ...
            if let Some(s) = fname.to_str() {
                if s.starts_with(&prefix) && s.ends_with(".bak") {
                    ...
                    if let Some(num) = s.strip_prefix(&prefix).and_then(|rest| rest.strip_suffix(".bak")).and_then(|n| n.parse::<u128>().ok()) {
...
pub fn replace_file_with_symlink(source: &Path, target: &Path, dry_run: bool, allow_degraded: bool, backup_tag: &str) -> std::io::Result<(bool, u64)> {
    if source == target { return Ok((false, 0)); }
    ...
    if is_symlink {
        ...
        if resolved_current == desired { return Ok((false, 0)); }
    ...
    if existed {
        if let Ok(ref meta) = metadata {
            ...
            let bname = backup_pb.file_name().and_then(|s| s.to_str()).unwrap_or("backup");
            let fname_c = std::ffi::CString::new(fname).unwrap();
            ...
            let dstfd = openat(&dirfd, bname_c.as_c_str(), OFlags::WRONLY | OFlags::CREATE | OFlags::TRUNC, Mode::from_bits_truncate(0o600))
                .map_err(|e| std::io::Error::from_raw_os_error(e.raw_os_error()))?;
            // Copy bytes
            ...
            let dstfd2 = openat(&dirfd, bname_c.as_c_str(), OFlags::RDONLY, Mode::empty())
                .map_err(|e| std::io::Error::from_raw_os_error(e.raw_os_error()))?;
            fchmod(&dstfd2, Mode::from_bits_truncate(mode)).map_err(|e| std::io::Error::from_raw_os_error(e.raw_os_error()))?;
            // Remove original target (will be replaced by atomic swap below)
            unlinkat(&dirfd, fname_c.as_c_str(), AtFlags::empty()).map_err(|e| std::io::Error::from_raw_os_error(e.raw_os_error()))?;
            let _elapsed_ms = t0.elapsed().as_millis() as u64;
            let _ = _elapsed_ms; // reserved for future telemetry
        }
    }
...
pub fn restore_file(target: &Path, dry_run: bool, force_best_effort: bool, backup_tag: &str) -> std::io::Result<()> {
