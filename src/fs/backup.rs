...
pub(crate) fn find_previous_backup_and_sidecar(target: &Path, tag: &str) -> Option<(Option<PathBuf>, PathBuf)> {
...
pub fn create_snapshot(target: &Path, backup_tag: &str) -> std::io::Result<()> {
    ...
    use rustix::fs::{openat, fchmod, AtFlags, OFlags, Mode};
    ...
    if is_symlink {
        ...
        if let Some(curr) = current_dest.as_ref() {
            ...
            write_sidecar(&backup, &sc).map_err(|e| std::io::Error::new(std::io::ErrorKind::Other, format!("sidecar write failed: {}", e)))?;
        }
        return Ok(());
    }
    ...
    if existed {
        if let Ok(ref meta) = metadata {
            ...
            write_sidecar(&backup_pb, &sc).map_err(|e| std::io::Error::new(std::io::ErrorKind::Other, format!("sidecar write failed: {}", e)))?;
            return Ok(());
        }
    }
