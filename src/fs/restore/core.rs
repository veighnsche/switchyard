...
/// Shared restore engine: select latest or previous pair and perform restore.
pub(crate) fn restore_internal(
    target: &SafePath,
    dry_run: bool,
    ...
    let pair = if use_previous {
        find_previous_backup_and_sidecar(&target_path, backup_tag)
    } else {
        find_latest_backup_and_sidecar(&target_path, backup_tag)
    };
