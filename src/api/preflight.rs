...
/// Export a PreflightReport to YAML containing only the core fields defined in SPEC/preflight.yaml.
/// The YAML is a sequence where each item is a map with keys:
/// action_id, path, current_kind, planned_kind, policy_ok, provenance (optional), notes (optional).
pub fn to_yaml(report: &crate::types::report::PreflightReport) -> String {
    ...
    for row in &report.rows {
        ...
        let keys = ["action_id","path","current_kind","planned_kind","policy_ok","provenance","notes"];
        for k in keys.iter() {
            let v = get(k);
            if !v.is_null() {
