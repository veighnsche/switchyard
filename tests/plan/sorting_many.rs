...
fn plan_sorts_mixed_actions_deterministically() {
    ...
    for i in (0..10).rev() { // reverse to randomize
    ...
    let plan = api.plan(PlanInput { link: link_reqs, restore: vec![RestoreRequest { target: SafePath::from_rooted(root, &restore_t).unwrap() }] });
    ...
    for act in plan.actions.iter() {
        match act {
            switchyard::types::plan::Action::EnsureSymlink { target, .. } => {
                assert_eq!(last_kind, 0u8, "links should come first until restore entries");
            ...
            switchyard::types::plan::Action::RestoreFromBackup { target } => {
                ...
                if last_kind == 0u8 { last_kind = 1u8; last_t.clear(); }
                let cur = target.rel().to_string_lossy().to_string();
                assert!(cur >= last_t, "restore targets sorted: {} >= {}", cur, last_t);
                last_t = cur;
            }
        }
    }
...
fn plan_many_restores_sorted() {
    ...
    for i in (0..10).rev() {
        ...
        restores.push(RestoreRequest { target: SafePath::from_rooted(root, &t).unwrap() });
    }
