...
fn restore_is_invertible_with_snapshot() {
    ...
    assert!(
        ...
        "target should be regular file after restore"
    );
    ...
    assert!(
        md3.file_type().is_symlink(),
        "target should be symlink after inverse restore"
    );
    let link = std::fs::read_link(&tgt).unwrap();
    assert!(
        link.ends_with("bin/new"),
        "inverse restore should re-symlink to new"
    );
}
