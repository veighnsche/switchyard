...
pub async fn then_signature_requires_safepath(_world: &mut World) {
    ...
    assert!(
        mentions_safe && has_prune,
        "expected prune_backups to take &SafePath"
    );
}
...
pub async fn then_toctou_sequence_present(_world: &mut World) {
    ...
    let has_open_dir =
        SNAPSHOT_RS.contains("open_dir_nofollow(") || ATOMIC_RS.contains("open_dir_nofollow(");
    let has_fsync_parent =
        SNAPSHOT_RS.contains("fsync_parent_dir(") || ATOMIC_RS.contains("fsync_parent_dir(");
