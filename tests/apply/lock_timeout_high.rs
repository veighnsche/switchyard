...
fn e2e_apply_010_lock_timeout_high() {
    ...
    let api = switchyard::Switchyard::new(facts.clone(), audit, policy).with_lock_timeout_ms(1000); // 1 second timeout
    ...
    if let Some(event) = lock_failure {
        ...
        assert!(
            lock_wait_ms <= 1000,
            "lock_wait_ms should be <= timeout (1000ms), got {}",
            lock_wait_ms
        );
        ...
        assert!(
            lock_attempts,
            "lock_attempts should be present in apply.attempt event"
        );
    }
}
