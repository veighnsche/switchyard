//! E2E-APPLY-015 — Lock contention timeout (REQ-L3)
//! Assert E_LOCKING error with bounded wait.
...
#[test]
fn e2e_apply_015_lock_contention_timeout() {
    // REQ-L3, E2E-APPLY-015 (P2)
    let facts = TestEmitter::default();
    let audit = JsonlSink::default();
    let mut policy = Policy::default();
    policy.governance.locking = switchyard::policy::types::LockingPolicy::Required;
    let api = switchyard::Switchyard::new(facts.clone(), audit, policy).with_lock_timeout_ms(500); // 500ms timeout
    ...
    assert!(
        apply_result.is_err(),
        "apply should fail when locking is required but no manager is configured"
    );
    ...
    if let Some(event) = lock_failure {
        let lock_wait_ms = event
            .get("lock_wait_ms")
            .and_then(|v| v.as_u64())
            .unwrap_or(0);
        assert!(
            lock_wait_ms <= 500,
            "lock_wait_ms should be <= timeout (500ms), got {}",
            lock_wait_ms
        );
    }
}
