...
fn restore_without_capture_snapshot_does_not_invert() {
    ...
    let plan = api.plan(PlanInput { link: vec![], restore: vec![RestoreRequest { target: sp_t.clone() }] });
    ...
    assert!(inv.actions.is_empty(), "expected no inverse actions when snapshot capture disabled");
}
