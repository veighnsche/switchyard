...
fn redaction_invariant() {
    ...
    assert!(dryrun_event.is_some(), "expected apply.result success event in dry run");
    ...
    if let Some(event) = dryrun_event {
        ...
        assert!(ts == "1970-01-01T00:00:00Z" || ts.is_empty(), "timestamp should be zeroed in dry run facts");
    }
}
