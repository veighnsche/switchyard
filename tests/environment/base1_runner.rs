...
fn envrunner_base1_ci_quick() {
    ...
    assert!(!plan.actions.is_empty(), "expected actions in plan for Base-1 environment");
    ...
    assert!(apply_result.is_ok(), "apply should succeed in Base-1 environment (dry run)");
}
