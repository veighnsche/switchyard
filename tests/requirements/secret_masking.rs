...
fn req_o6_secret_masking() {
    ...
    for event in redacted.iter() {
        ...
        if let Some(path) = event.get("path").and_then(|v| v.as_str()) {
            ...
            assert!(!path.starts_with("/etc/"), "paths should not expose sensitive system directories");
            assert!(!path.starts_with("/root/"), "paths should not expose sensitive system directories");
        }
    }
