...
fn req_r5_partial_restoration_facts() {
    ...
    let input = PlanInput { 
        link: vec![
            LinkRequest { source: s1, target: t1 },
            LinkRequest { source: s2, target: t2 }
        ], 
        restore: vec![] 
    };
    ...
    let rollback_events = redacted.iter().filter(|e| {
        e.get("stage").and_then(|v| v.as_str()).unwrap_or("").contains("rollback")
    }).count();
    
    // In dry run mode, we can verify that rollback planning occurs
    assert!(!rollback_plan.actions.is_empty(), "rollback plan should contain actions");
