...
fn req_l4_lockmanager_required_production() {
    ...
    assert!(apply_result.is_err(), "commit should fail when locking is required but no manager is configured");
    ...
    assert!(lock_error_event.is_some(), "expected E_LOCKING error when lock manager is required but absent");
}
