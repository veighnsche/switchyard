...
fn req_s6_backup_sidecar_integrity() {
    ...
    assert!(apply_result.errors.is_empty(), "dry run should not have errors with sidecar integrity enabled");
    ...
    // No errors should occur in dry run mode
    assert!(apply_result.errors.is_empty(), "dry run should not have errors with sidecar integrity disabled");
}
